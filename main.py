...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good morning Hemanth, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon Hemanth, it's {day} and the time is {t}")
    else:
        speak(f"Good evening Hemanth, it's {day} and the time is {t}")
...
def browsing(query):
    if 'google' in query:
        ...
        s = command().lower()  # Get the search query
        search_query = quote(s)  # Encode the search query
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if ('facebook' in query) or ('discord' in query) or ('whatsapp' in query) or ('instagram' in query):
            ...
            speak("Volume muted")
        elif ("open calculator" in query) or ("open notepad" in query) or ("open excel" in query):
            openApp(query)
        elif ("close calculator" in query) or ("close notepad" in query) or ("close excel" in query):
            closeApp(query)
